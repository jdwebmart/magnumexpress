(function ($) {
  ...
  // sticky header
  if (localize_params.sticky_header == 1) {
  ...
  // Tracking ajax 
  $('#logistics-tracking-form').on('submit', function (e) {
    e.preventDefault();
    var tracking_id = $('#tracking_id').val().trim();
    if (!tracking_id) {
      alert('Please enter your tracking ID');
      return;
    }
    $('#tracking-result').html('<p>Checking...</p>');
    $.ajax({
      url: localize_params.ajaxurl,
      type: 'POST',
      data: {
        action: 'logistics_track_shipment',
      ...
      error: function () {
        $('#tracking-result').html('<p style="color:red;">Error occurred. Please try again.</p>');
      }
    });
  });
  ...
  function filterCareers() {
    ...
    $.ajax({
      ...
      beforeSend: function () {
        $('#career-results').html('<p>Loading...</p>');
      },
      success: function (response) {
  ...
  $('#filter-location, #filter-category, #filter-language').on('change', filterCareers);
